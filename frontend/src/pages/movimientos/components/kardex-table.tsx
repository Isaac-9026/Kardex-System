...
const getSemaforo = (row: KardexRow) => {
  ...
  if (row.costo_reconstruido)
    return { badge: "🟦", text: "Costo Reconstruido" };
  return { badge: "🟢", text: "Conforme" };
};

const getRowTooltipDescription = (row: KardexRow): string => {
  if (row.saldo_negativo)
    return "Se registró una salida cuando el producto ya no tenía stock suficiente.";
  if (row.sin_saldo_inicial)
    return "El producto no tiene saldo inicial registrado.";
  if (row.error_b)
    return "El Excel presenta diferencias matemáticas entre cantidad, costo unitario y total.";
  if (row.error_a)
    return "El valor original estaba vacío o incompleto; el sistema reconstruyó el costo.";
  if (row.costo_reconstruido)
    return "El sistema completó el costo porque el Excel no traía un valor confiable.";
...
export const KardexTable = forwardRef<KardexTableHandle, KardexTableProps>(
  function KardexTable(
    ...
    const findPrimerIndexCodigo = (codigo: string) =>
      movimientos.findIndex(
        (m) => String(m.codigo).trim() === String(codigo).trim(),
      );
    ...
    const bloquesPrint = useMemo(
      () => agruparPorProductoMes(movimientos),
      [movimientos],
    );
    ...
    main, .main-content {
      ...
      width: 100% !important;
          ...
          .kp-mes-block { page-break-inside: avoid; break-inside: avoid; margin-bottom: 20px; }
          ...
          .kp-mes-titulo { font-family: Arial, sans-serif; font-size: 13px; font-weight: 900; text-align: center; text-transform: uppercase; letter-spacing: .04em; margin: 8px 0 5px; }
          .kp-prod-info { width: 100%; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 8px; margin-bottom: 3px; border-top: 2px solid #222; border-bottom: 2px solid #222; }
          ...
          .kp-mov-table { width: 100%; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 7.5px; }
          .kp-mov-table th, .kp-mov-table th.th-grupo { background: white !important; color: black !important; border: 1px solid #333 !important; padding: 3px 4px !important; font-weight: 700 !important; font-size: 7px !important; text-transform: uppercase !important; text-align: center !important; }
          .kp-mov-table .th-grupo { border-bottom: 2px solid #333 !important; }
          .kp-mov-table td { border: 1px solid #ccc; padding: 2px 4px; color: black !important; background: white !important; white-space: nowrap; }
      ...
      `}</style>
        ...
        {/* ════ PANEL WEB DE AUDITORÍA (Mapeo Simétrico Corregido) ════ */}
          ...
          <div className="rounded-xl border bg-card/30 backdrop-blur-md text-card-foreground shadow-2xs overflow-x-auto border-border/50">
            <Table className="text-[11px] min-w-[1550px] table-fixed">
              <TableHeader className="bg-muted/20 text-xs border-b border-border/50">
                {/* CABECERA FILA 1: Macro-grupos contables */}
                <TableRow className="hover:bg-transparent border-b border-border/40">
                  {mostrarSemaforo && (
                    <TableHead className="w-[50px] text-center text-muted-foreground font-bold">
                      EST
                    </TableHead>
                  )}
                  <TableHead className="w-[50px] text-center text-muted-foreground font-bold">
                  ...
                  <TableHead className="w-[90px] text-left text-muted-foreground font-bold">
                    Cód.
                  </TableHead>
                  <TableHead
                    colSpan={4}
                    className="text-center bg-blue-600/10 text-blue-400 font-bold border-r border-border/30"
                  >
                    Comprobante Fiscal
                  </TableHead>
                  <TableHead className="w-[140px] text-center bg-emerald-600/10 text-emerald-400 font-bold border-r border-border/30">
                    Operación
                  </TableHead>
                  <TableHead
                    colSpan={3}
                    className="text-center bg-teal-600/10 text-teal-400 font-bold border-r border-border/30"
                  >
                    Entradas
                  </TableHead>
                  <TableHead
                    colSpan={3}
                    className="text-center bg-rose-600/10 text-rose-400 font-bold border-r border-border/30"
                  >
                    Salidas
                  </TableHead>
                  <TableHead
                    colSpan={3}
                    className="text-center bg-sky-600/10 text-sky-400 font-bold"
                  >
                    Saldo Final Acumulado
                  </TableHead>
                </TableRow>

                {/* CABECERA FILA 2: Subdivisiones por columna exacta */}
                <TableRow className="hover:bg-transparent bg-muted/10 text-[10px] uppercase tracking-wider text-muted-foreground/80 border-b border-border/30">
                  ...
                  {/* Comprobante */}
                  <TableHead className="w-[90px] py-2 text-center">
                    Fecha
                  </TableHead>
                  <TableHead className="w-[50px] py-2 text-center">
                    Tipo
                  </TableHead>
                  <TableHead className="w-[60px] py-2 text-center">
                    Serie
                  </TableHead>
                  <TableHead className="w-[140px] py-2 text-center border-r border-border/30">
                    Número
                  </TableHead>
                  {/* Operación */}
                  <TableHead className="w-[140px] py-2 text-left border-r border-border/30">
                    Tipo de Operación
                  </TableHead>
                  {/* Entradas */}
                  <TableHead className="w-[80px] py-2 text-right">
                    Cant
                  </TableHead>
                  <TableHead className="w-[90px] py-2 text-right">
                    C.Unit
                  </TableHead>
                  <TableHead className="w-[100px] py-2 text-right border-r border-border/30">
                    Total
                  </TableHead>
                  {/* Salidas */}
                  <TableHead className="w-[80px] py-2 text-right">
                  ...
                  <TableHead className="w-[100px] py-2 text-right">
                    Total
                  </TableHead>
                </TableRow>
              </TableHeader>
                  ...
                  {filas.map((row, i) => {
                    if (row.es_saldo_inicial) {
                      return (
                        <TableRow
                          key="saldo-inicial"
                          className="bg-blue-500/5 dark:bg-blue-500/10 hover:bg-blue-500/10 border-l-4 border-l-blue-500 transition-colors"
                        >
                          {mostrarSemaforo && (
                            <TableCell className="w-[50px] text-center font-bold text-muted-foreground/40">
                              —
                            </TableCell>
                          )}
                          <TableCell className="w-[50px] text-center font-bold text-blue-500 dark:text-blue-400">
                            —
                          </TableCell>
                          <TableCell className="w-[90px] font-semibold text-blue-600 dark:text-blue-400">
                            {row.codigo}
                          </TableCell>
                          <TableCell className="w-[90px] text-center text-blue-600 dark:text-blue-400 font-medium">
                            {fmtFecha(row.fecha)}
                          </TableCell>
                          <TableCell className="w-[50px] text-center text-muted-foreground/20">
                            —
                          </TableCell>
                          <TableCell className="w-[60px] text-center text-muted-foreground/20">
                            —
                          </TableCell>
                          <TableCell className="w-[140px] text-center text-muted-foreground/20 border-r border-border/30">
                            —
                          </TableCell>
                          <TableCell className="w-[140px] text-left text-blue-600 dark:text-blue-400 font-bold uppercase tracking-wide border-r border-border/30">
                            SALDO INICIAL
                          </TableCell>
                          <TableCell className="w-[80px] text-muted-foreground/20">
                            —
                          </TableCell>
                          <TableCell className="w-[90px] text-muted-foreground/20">
                            —
                          </TableCell>
                          <TableCell className="w-[100px] text-muted-foreground/20 border-r border-border/30">
                            —
                          </TableCell>
                          <TableCell className="w-[80px] text-muted-foreground/20">
                            —
                          ...
                          <TableCell className="w-[80px] text-right font-bold text-blue-600 dark:text-blue-400">
                            {fmtCant(row.saldo_cantidad)}
                          </TableCell>
                          <TableCell className="w-[90px] text-muted-foreground/20">
                            —
                          </TableCell>
                          <TableCell className="w-[100px] text-right font-bold text-blue-600 dark:text-blue-400">
                    ...
                    return (
                      <Tooltip key={row.id}>
                        <TooltipTrigger asChild>
                          <TableRow
                            ...
                            className={cn(
                              ...
                              esHighlight &&
                                "bg-blue-500/15 dark:bg-blue-500/20 hover:bg-blue-500/25 ring-2 ring-blue-500/40",
                              !esHighlight &&
                                esError &&
                              ...
                              !esHighlight &&
                                !esError &&
                                tieneError &&
                                "bg-red-500/5 dark:bg-red-500/10 hover:bg-red-500/15",
                              !esHighlight &&
                                !esError &&
                                !tieneError &&
                                "hover:bg-muted/30 odd:bg-transparent even:bg-muted/5 dark:even:bg-muted/10",
                            )}
                          >
                            {mostrarSemaforo && (
                              <TableCell className="w-[50px] text-center text-sm">
                                {semaforo.badge}
                              </TableCell>
                            )}
                            <TableCell className="w-[50px] text-center font-mono text-muted-foreground/60 dark:text-muted-foreground/80">
                              {row.fila}
                            </TableCell>
                            <TableCell className="w-[90px] font-semibold text-blue-600 dark:text-blue-400">
                              {row.codigo}
                            </TableCell>
                            <TableCell className="w-[90px] text-center text-foreground/80 dark:text-muted-foreground">
                              {fmtFecha(row.fecha)}
                            </TableCell>
                            <TableCell className="w-[50px] text-center font-mono">
                              {row.tipo_comprobante}
                            </TableCell>
                            <TableCell className="w-[60px] text-center font-mono">
                              {row.serie}
                            </TableCell>
                            <TableCell className="w-[140px] text-center font-mono border-r border-border/30">
                              {row.numero}
                            </TableCell>
                            <TableCell className="w-[140px] text-left text-foreground/90 dark:text-foreground/90 font-medium truncate border-r border-border/30">
                              {row.tipo_operacion}
                            </TableCell>
                            <TableCell className="w-[80px] text-right font-mono text-foreground/90 dark:text-muted-foreground">
                              {row.ent_cantidad > 0
                                ? fmtCant(row.ent_cantidad)
                                : "—"}
                            </TableCell>
                            <TableCell className="w-[90px] text-right font-mono text-muted-foreground/60 dark:text-muted-foreground/70">
                              {row.ent_costo_unit > 0
                                ? fmtUnit(row.ent_costo_unit)
                                : "—"}
                            </TableCell>
                            <TableCell className="w-[100px] text-right font-mono text-foreground/90 dark:text-muted-foreground border-r border-border/30">
                              {row.ent_costo_total > 0
                                ? fmtTotal(row.ent_costo_total)
                                : "—"}
                            </TableCell>
                            <TableCell className="w-[80px] text-right font-mono text-foreground/90 dark:text-muted-foreground">
                              {row.sal_cantidad > 0
                                ? fmtCant(row.sal_cantidad)
                                : "—"}
                            </TableCell>
                            <TableCell className="w-[90px] text-right font-mono text-muted-foreground/60 dark:text-muted-foreground/70">
                              {row.sal_costo_unit > 0
                                ? fmtUnit(row.sal_costo_unit)
                                : "—"}
                            </TableCell>
                            <TableCell className="w-[100px] text-right font-mono text-foreground/90 dark:text-muted-foreground border-r border-border/30">
                              {row.sal_costo_total > 0
                                ? fmtTotal(row.sal_costo_total)
                                : "—"}
                            </TableCell>

                            {/* 🟢 COLUMNAS DE CONTROL DE SALDO INTEGRADAS PARA LIGHT Y DARK MODE */}
                            <TableCell
                              className={cn(
                                "w-[80px] text-right font-mono font-semibold",
                                row.saldo_negativo
                            ...
                            <TableCell
                              className={cn(
                                "w-[100px] text-right font-mono font-semibold",
                                row.saldo_negativo
                                  ? "text-red-500 dark:text-red-400"
                                  : "text-blue-600 dark:text-sky-400",
                              )}
                        ...
                        {tieneError && (
                          <TooltipContent
                            side="top"
                            className="max-w-xs p-3 bg-zinc-950 dark:bg-zinc-900 border border-border text-white rounded-xl shadow-xl"
                          >
                ...
                <div key={mes.mesKey} className="kp-mes-block">
                  <div className="kp-empresa">
                    <div className="kp-empresa-field">
                      ...
                      <span className="kp-empresa-val">
                        {empresaImpresion?.razon_social ?? ""}
                      </span>
                    </div>
                    <div className="kp-empresa-field">
                      <span className="kp-empresa-lbl">Ruc:</span>
                      <span className="kp-empresa-val">
                        {empresaImpresion?.ruc ?? ""}
                      </span>
                    </div>
                    <div className="kp-empresa-field">
                      <span className="kp-empresa-lbl">Dirección:</span>
                      <span className="kp-empresa-val">
                        {empresaImpresion?.establecimiento ?? ""}
                      </span>
                    </div>
                  </div>
                  ...
                  <table className="kp-prod-info">
                    <tbody>
                      <tr>
                        ...
                        <td className="val">
                          {empresaImpresion?.tipo ?? "Mercadería"}
                        </td>
                        <td className="lbl">Costo:</td>
                        <td className="val">
                          {empresaImpresion?.metodo_valuacion ??
                            "Prom. Ponderado"}
                        </td>
                      </tr>
                    </tbody>
                  ...
                  <table className="kp-mov-table">
                    <thead>
                      <tr>
                        ...
                        <th className="th-grupo" colSpan={3}>
                          COMPROBANTE
                        </th>
                        <th
                          className="th-grupo"
                    ...
                    <tbody>
                      <tr className="tr-saldo">
                        ...
                        <td className="td-r">—</td>
                        <td className="td-r">
                          {fmtCant(mes.saldoAnteriorCant)}
                        </td>
                        <td className="td-r">
                          {fmtUnit(mes.saldoAnteriorUnit)}
                        </td>
                        <td className="td-r">
                          {fmtTotal(mes.saldoAnteriorTotal)}
                        </td>
                      </tr>
                      {mes.filas.map((row, ri) => (
                        <tr
                          key={row.id}
                          className={ri % 2 !== 0 ? "tr-alt" : ""}
                        >
                      ...
                      {(() => {
                        const totEntCant = mes.filas.reduce(
                          (s, r) => s + (r.ent_cantidad || 0),
                          0,
                        );
                        const totEntTotal = mes.filas.reduce(
                          (s, r) => s + (r.ent_costo_total || 0),
                          0,
                        );
                        const totSalCant = mes.filas.reduce(
                          (s, r) => s + (r.sal_cantidad || 0),
                          0,
                        );
                        const totSalTotal = mes.filas.reduce(
                          (s, r) => s + (r.sal_costo_total || 0),
                          0,
                        );
                        ...
                        return (
                          <tr className="tr-total">
                            ...
                            <td className="td-r">
                              {fmtCant(ultima?.saldo_cantidad ?? 0)}
                            </td>
                            <td className="td-r">
                              {fmtUnit(ultima?.saldo_costo_unit ?? 0)}
                            </td>
                            <td className="td-r">
                              {fmtTotal(ultima?.saldo_costo_total ?? 0)}
                            </td>
                          </tr>
                        );
                      })()}
