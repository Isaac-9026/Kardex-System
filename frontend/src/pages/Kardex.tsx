...
const IconSpinner = () => (
  <svg
    ...
    viewBox="0 0 24 24"
    fill="none"
    style={{ animation: "kspin 1s linear infinite" }}
  >
    <circle
      cx="12"
      cy="12"
...
const Sidebar = ({ id, onNavigate, currentPath }: SidebarProps) => {
  const navItem = (
    ...
    >
      ...
      {!dot && (
        <span style={{ color: active ? "#60a5fa" : "#3a5a7a", flexShrink: 0 }}>
          {icon}
        </span>
      )}
      {label}
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          height: 1,
          background: "rgba(56,139,221,0.08)",
          margin: "10px 0",
        }}
      />
...
/* ═══════════════════════════ Formatters ═══════════════════════════ */
const _nf2 = new Intl.NumberFormat("es-PE", {
...
export default function Kardex() {
  ...
  const filtrosAplicadosTexto = useMemo(() => {
    ...
    if (filtroFecha.modo === "anio_mes") {
      if (filtroFecha.anio && filtroFecha.mes) {
        ...
        partes.push(
          `Periodo: ${meses[filtroFecha.mes - 1]} ${filtroFecha.anio}`,
        );
      ...
      if (filtroFecha.fecha_desde || filtroFecha.fecha_hasta) {
        partes.push(
          `Rango: ${filtroFecha.fecha_desde ?? "..."} a ${filtroFecha.fecha_hasta ?? "..."}`,
        );
      }
    }
  ...
  // ✅ ENFOQUE UNIVERSAL REPARADO: Sincroniza la metadata evitando variables no definidas
  useEffect(() => {
    ...
    // Buscamos el movimiento que coincida con el código activo, o tomamos el primero
    const movimientoMeta =
      movimientos.find((m) => m.codigo === codigo) || movimientos[0];

    if (movimientoMeta?.producto) {
      const prodExistente = movimientoMeta.producto;
      const emp = prodExistente.empresa;

      setEmpresaImpresion({
        razon_social: emp?.nombre || "⚠️ PENDIENTE DE ASIGNAR EMPRESA",
        ruc: emp?.ruc || "00000000000",
        direccion: emp?.direccion || "—",
        codigo_existencia: prodExistente.codigo || "—",
        nombre_producto: prodExistente.descripcion || "—",
        unidad_medida: prodExistente.unidad_medida || "NIU",
      });
    } else {
      // FALLBACK SEGURO: Si no hay producto asociado aún, mapeamos desde las propiedades raíz del movimiento
      setEmpresaImpresion({
      ...
      >
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              {erroresIntegridad > 0 && (
                ...
                >
                  ⚠ {erroresIntegridad} anomalía
                  {erroresIntegridad > 1 ? "s" : ""}
                </button>
              )}
              <button
                type="button"
                ...
                style={{
                  ...
                  background: filtrosAbiertos
                    ? "rgba(56,139,221,0.15)"
                    : "rgba(56,139,221,0.06)",
                  border: filtrosAbiertos
                    ? "1px solid rgba(56,139,221,0.35)"
              ...
              <button
                ...
                style={{
                  ...btnBase,
                  background: mostrarSemaforo
                    ? "rgba(245,158,11,0.12)"
                    : "rgba(56,139,221,0.06)",
                  border: mostrarSemaforo
              ...
              <button
                ...
                style={{
                  ...
                  cursor:
                    exporting || movimientos.length === 0
                      ? "not-allowed"
                      : "pointer",
                }}
              >
          ...
          >
              ...
              <div className="hdr-wrap">
                ...
                <div className="hdr-top">
                  ...
                  <table className="hdr-periodo-box">
                    <thead>
                      <tr>
                        <th colSpan={3}>PERIODO DE REPORTE</th>
                      </tr>
                      <tr>
                        <th>DÍA</th>
                        <th>MES</th>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>
                          {filtroFecha.mes?.toString().padStart(2, "0") ??
                            (new Date().getMonth() + 1)
                              .toString()
                              .padStart(2, "0")}
                        </td>
                ...
                <table className="hdr-campos">
                  <tbody>
                    ...
                    <tr>
                      <td className="hdr-lbl">Dirección:</td>
                      <td className="hdr-val" colSpan={4}>
                        {empresaImpresion?.direccion ?? "—"}
                      </td>
                    </tr>
                    <tr>
                      <td className="hdr-lbl">Código:</td>
                      <td
                        className="hdr-val"
                        style={{ fontWeight: 700, fontFamily: "monospace" }}
                      >
                        {empresaImpresion?.codigo_existencia ?? ""}
                      </td>
                      <td></td>
                      <td className="hdr-lbl">Producto:</td>
                      <td className="hdr-val" style={{ fontWeight: 700 }}>
                        {empresaImpresion?.nombre_producto ?? "—"}
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            {metricas && (
              <div
                className="kardex-no-print kardex-metrics-web"
            ...
            {metricas && (
              <div className="kardex-print-only kardex-print-metrics">
                ...
                <div>
                  ...
                  <div className="sub">
                    {fmt(metricas.total_ent_cantidad)} uds
                  </div>
                </div>
                <div>
                  ...
                  <div className="sub">
                    {fmt(metricas.total_sal_cantidad)} uds
                  </div>
                </div>
                <div>
                  ...
                  <div className="sub">
                    {fmt(metricas.saldo_final_cantidad)} uds
                  </div>
                </div>
              </div>
            )}
            ...
            {filtrosAbiertos && (
              ...
              >
                ...
                >
                  <span
                    style={{ fontSize: 10, color: "#2a5a7a", flexShrink: 0 }}
                  >
                    Código
                  </span>
                  <input
                ...
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  {(["anio_mes", "exacta", "rango"] as const).map((m) => (
                    <button
                      key={m}
                      onClick={() =>
                        setDraftFiltroFecha({ ...draftFiltroFecha, modo: m })
                      }
                      style={{
                        padding: "3px 8px",
                        borderRadius: 4,
                        ...
                        background:
                          draftFiltroFecha.modo === m
                            ...
                            : "rgba(56,139,221,0.06)",
                        color:
                          draftFiltroFecha.modo === m ? "#60a5fa" : "#2a5a7a",
                        fontSize: 10,
                        fontWeight: draftFiltroFecha.modo === m ? 600 : 400,
                        cursor: "pointer",
                        fontFamily: "inherit",
                      }}
                    >
                ...
                {draftFiltroFecha.modo === "exacta" && (
                  <input
                    ...
                    onChange={(e) =>
                      setDraftFiltroFecha({
                        ...draftFiltroFecha,
                        fecha_exacta: e.target.value || undefined,
                      })
                    }
            ...
            >
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  >
                    ...
                    />
                    <span style={{ fontSize: 10, color: "#1e3a5a" }}>
                      En línea
                    </span>
                  </div>
                </div>
              ...
              {movimientos.length > 0 && (
                ...
                >
                  ...
                  >
                    Mostrando {movimientos.length.toLocaleString("es-PE")} de{" "}
