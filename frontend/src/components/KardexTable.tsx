        ...
        <table className="min-w-full text-xs">
          ...
          <tbody className="divide-y divide-gray-100">
            {filas.map((row, i) => {
              ...
              return (
                <tr key={row.id} className={`${colorFila} hover:bg-indigo-50/40 transition-colors`}>
                  {mostrarSemaforo && (
                    <td className={tdClass + ' text-center text-base'}>{row.semaforo}</td>
                  )}
                  <td className={tdClass + ' font-mono font-semibold text-indigo-700'}>
                    {row.codigo ?? '—'}
                  </td>
                  <td className={tdClass + ' text-center whitespace-nowrap'}>
                    {fmtFecha(row.fecha)}
                  </td>
                  <td className={tdClass + ' text-center'}>{row.tipo_comprobante}</td>
                  <td className={tdClass + ' text-center'}>{row.serie}</td>
                  <td className={tdClass + ' text-center font-mono'}>{row.numero}</td>
                  <td className={tdClass + ' whitespace-nowrap'}>{row.tipo_operacion}</td>
                  {/* Entradas */}
                  <td className={tdNumClass}>{fmtCant(row.ent_cantidad)}</td>
                  <td className={tdNumClass}>{fmtUnit(row.ent_costo_unit)}</td>
                  <td className={tdNumClass}>{fmtTotal(row.ent_costo_total)}</td>
                  {/* Salidas */}
                  <td className={tdNumClass}>{fmtCant(row.sal_cantidad)}</td>
                  <td className={tdNumClass}>{fmtUnit(row.sal_costo_unit)}</td>
                  <td className={tdNumClass}>{fmtTotal(row.sal_costo_total)}</td>
                  {/* Saldo */}
                  <td className={tdNumClass + ' font-semibold'}>{fmtCant(row.saldo_cantidad)}</td>
                  <td className={tdNumClass + ' font-semibold'}>{fmtUnit(row.saldo_costo_unit)}</td>
                  <td className={tdNumClass + ' font-semibold'}>{fmtTotal(row.saldo_costo_total)}</td>
                </tr>
              )
            })}
          </tbody>
...
const thClass    = 'px-2 py-2 text-center border-r border-indigo-200 last:border-r-0 whitespace-nowrap'
const tdClass    = 'px-2 py-1.5 border-r border-gray-100 last:border-r-0'
const tdNumClass = 'px-2 py-1.5 text-right border-r border-gray-100 last:border-r-0 tabular-nums'